...
var CraftingStateManager$1 = new CraftingStateManager();class GuideButtonManager {
    constructor() {
        ...
        this.buildButton = (screen, buttons) => {
            this.clearRenderedButtons(screen);
            ...
            const quantityButtons = visibleButtons.filter((button) => button.quantityGrid && !button.quantityReset);
            const resetButton = visibleButtons.find((button) => button.quantityReset);
            ...
            if (resetButton) {
                const resetX = this.startX + Math.floor((this.BUTTON_WIDTH + 5) / 2);
                const resetY = gridStartY + Math.ceil(quantityButtons.length / 2) * (this.BUTTON_HEIGHT + 5);
                this.buildSingleButton(screen, resetButton, resetX, resetY);
            }
        };
        this.clearRenderedButtons = (screen) => {
            const rendered = this._rendered.get(screen) || [];
            rendered.forEach((button) => screen.removeButton(button));
            this._rendered.delete(screen);
        };
        this.buildSingleButton = (screen, button, x, y) => {
            ...
            const rendered = this._rendered.get(screen) || [];
            rendered.push(built);
            this._rendered.set(screen, rendered);
        };
        ...
        this.refresh = (screen, inventory) => {
            if (!screen || this._processing)
                return;
            const screenClassName = screen.getClass().getSimpleName();
    ...
    clear() {
        this._event?.off();
        this._rendered.forEach((buttons, screen) => buttons.forEach((button) => screen.removeButton(button)));
...
var GuideButtonManager$1 = new GuideButtonManager();class MaterialCalculator {
            ...
            if (craftCount > 0) {
                ...
                for (const ingredient of recipe.ingredients) {
                    ...
                    requirements.set(ingredient.id, {
                        ...
                        count: newCount,
                        maxCount: ingredient.maxCount || existingReq?.maxCount,
    ...
    optimizeInventoryAllocation(requirements) {
        ...
        for (const [itemId, requirement] of requirements) {
            const recipe = RecipeRepository$1.getRecipe(itemId);
            const { name: itemName, count: required, maxCount } = requirement;
            ...
            if (isBasicMaterial && actualMissing > 0) {
                missing.add(itemId, itemName, actualMissing, { maxCount });
            }
    ...
    addMaterialRequirement(requirements, itemId, itemName, count, maxCount) {
        const existing = requirements.get(itemId);
        requirements.set(itemId, {
            id: itemId,
            name: itemName,
            count: (existing?.count || 0) + count,
            maxCount: maxCount || existing?.maxCount,
    ...
    processBasicMaterial(itemId, needed, requirements) {
        ...
        if (actualNeed > 0) {
            ...
            this.addMaterialRequirement(requirements, itemId, itemName, actualNeed, recipe?.maxCount);
...
var MaterialCalculator$1 = new MaterialCalculator();class SlimefunGuideManager {
    constructor() {
        ...
        this.addButtons = () => {
            ...
            buttons.push(this.quantityBtn('数量-64', -64));
            buttons.push(this.quantityResetBtn());
        ...
        this.blacklistBtn = (operation, text) => {
            ...
            const callback = (btn) => {
                ...
                btn.setActive(false);
                GuideButtonManager$1.refresh(Hud.getOpenScreen(), Player.openInventory());
            };
            return { text, showCondition, callback };
        };
        this.quantityBtn = (text, countDelta) => {
            ...
            const callback = (btn) => {
                ...
                if (!recipeDetail)
                    return;
                const outputItem = InventoryHelper$1.waitForItemInSlot(recipeDetail.outputSlot, undefined, 500);
                if (!outputItem || outputItem.isEmpty()) {
                    Logger$1.warn('当前配方产物不可用，无法调整制作数量');
                    return;
                }
                btn.setActive(false);
                AutoCrafterManager$1.modifyMakeTable(outputItem.id, outputItem.name, countDelta, outputItem.maxCount);
                btn.setActive(true);
            };
            return { text, showCondition, callback, quantityGrid: true };
        };
        this.quantityResetBtn = () => {
            const text = '数量归零';
            const showCondition = () => {
                const recipeDetail = this.getRecipeDetail();
                return !!recipeDetail && !recipeDetail.outputItem.isEmpty();
            ...
            const callback = (btn) => {
                ...
                if (!outputItem || outputItem.isEmpty())
                    return;
                btn.setActive(false);
                AutoCrafterManager$1.resetMakeTableItem(outputItem.id, outputItem.name);
                btn.setActive(true);
            };
            return { text, showCondition, callback, quantityGrid: true, quantityReset: true };
        };
        this.readCraftingRecipe = () => {
            ...
            for (let index = 0; index < recipeDetail.materialItems.length; index++) {
                ...
                materials.push({
                    ...
                    maxCount: materialItem.maxCount,
                    slot: index,
                });
            }
            const craftingRecipe = {
                ...
                maxCount: recipeDetail.outputItem.maxCount,
            };
...
var SlimefunGuideManager$1 = new SlimefunGuideManager();class HudManager {
    constructor() {
        ...
        this.initHud = () => {
            ...
            this.drawText(`§b◆ §f${AppConfig.script.name} §b${AppConfig.script.version}`, startX, top - lineHeight - margin, AppConfig.ui.colors.brand);
            this.drawText(`§7维护者 §f${AppConfig.script.maintainer}`, startX, top - lineHeight * 2 - margin, AppConfig.ui.colors.muted);
            this.draw(this._make, '§a▌§f制作需求 §7[§a0§7]', top, middle - margin, startX, lineHeight, AppConfig.ui.colors.make);
            this.draw(this._missing, '§c▌§f缺失材料 §7[§c0§7]', middle, bottom - margin, startX, lineHeight, AppConfig.ui.colors.missing);
            this.draw(this._queue, '§b▌§f合成队列 §7[§b0§7]', top, bottom - margin, startX + margin + colWidth, lineHeight, AppConfig.ui.colors.queue);
        };
        ...
        this.updateMakeHud = (list) => {
            EventHelper$1.once('Tick', () => {
                ...
                this._make.title.setText(`§l§a▌§f制作需求 §7[§a${list.length}§7]`);
                ...
                for (const item of list) {
                    const groups = this.formatGroups(item.id, item.count, item.extra?.maxCount);
                    this._make.list[i].setText(`§f${this.compactName(item.name)} §8· §a§l×${item.count} §7${groups}`);
                    if (++i === len)
                        break;
                }
            });
        ...
        this.updateMissingHud = (list) => {
            EventHelper$1.once('Tick', () => {
                ...
                this._missing.title.setText(`§l§c▌§f缺失材料 §7[§c${list.length}§7]`);
                if (list.isEmptyOrZero()) {
                ...
                for (const item of list) {
                    const groups = this.formatGroups(item.id, item.count, item.extra?.maxCount);
                    this._missing.list[i].setText(`§f${this.compactName(item.name)} §8· §c§l缺${item.count} §7${groups}`);
        ...
        this.updateQueueHud = (list) => {
            EventHelper$1.once('Tick', () => {
                ...
                this._queue.title.setText(`§l§b▌§f合成队列 §7[§b${list.length}§7]`);
                if (list.isEmptyOrZero()) {
                ...
                for (const item of list) {
                    const groups = this.formatGroups(item.id, item.count, item.extra?.maxCount);
                    this._queue.list[i].setText(`§f${this.compactName(item.name)} §8· §b§l×${item.count} §7${groups}`);
                    if (++i === len - 1 && list.length > len) {
                        const remainNum = list.length - len + 1;
    ...
    resolveMaxCount(itemId, storedMaxCount) {
        const saved = Number(storedMaxCount);
        if (Number.isFinite(saved) && saved > 0)
            return saved;
        const inventoryItem = InventoryHelper$1.getItems('main', 'hotbar', 'offhand').find((item) => item.id === itemId);
        return Number(inventoryItem?.maxCount || AppConfig.game.maxItemStackSize);
    }
    formatGroups(itemId, count, storedMaxCount) {
        const maxCount = this.resolveMaxCount(itemId, storedMaxCount);
        if (maxCount <= 1)
            return '（不可堆叠）';
        const groups = Math.floor(count / maxCount);
        const remainder = count % maxCount;
        return `（${groups}组${remainder > 0 ? `+${remainder}` : ''}）`;
    }
    compactName(name, maxLength = 10) {
        const text = String(name || '').replace(/§[0-9a-fk-or]/gi, '');
...
var InventorySpaceChecker$1 = new InventorySpaceChecker();class AutoCrafterManager {
    constructor() {
        ...
        this.resetMakeTableItem = (itemId, itemName) => {
            const normalizeName = (name) => String(name || '').replace(/§[0-9a-fk-or]/gi, '').trim();
            const recipe = RecipeRepository$1.getRecipe(itemId);
            const targetName = normalizeName(recipe?.name || itemName);
            const entry = this.makeList.get(itemId)
                || this.makeList.all().find((item) => normalizeName(item.name) === targetName);
            if (!entry || entry.count <= 0) {
                Logger$1.info(`制作数量已经为 0: ${itemName}`);
                return false;
            }
            this.makeList.remove(entry.id, entry.count);
            this.redrawHud(true, false, false);
            this.calcDebounce.debounce(() => this.calculate());
            Logger$1.success(`制作数量已归零: ${entry.name}`);
            return true;
        };
        this.modifyMakeTable = (itemId, itemName, countDelta, maxCount) => {
            if (countDelta > 0) {
                const recipe = RecipeRepository$1.getRecipe(itemId);
                if (!recipe)
                ...
                const stackSize = Number(maxCount || recipe.maxCount || AppConfig.game.maxItemStackSize);
                this.makeList.add(itemId, recipe.name, countDelta, { maxCount: stackSize });
            }
            else {
        ...
        switch (event.slot) {
            ...
            case res.outputSlot:
                if (item && !item.isEmpty()) {
                    ...
                    if (itemCountDelta !== 0) {
                        this.modifyMakeTable(item.id, item.name, itemCountDelta, item.maxCount);
                    }
                }
                break;
        }
